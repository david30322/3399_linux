...
bool stk_als_swr = false;
extern bool stk_ps_swr;
...
static int stk_light_sensor_init(struct i2c_client *client)
{
    ...
    reg_num = sizeof(stk3x3x_als_config_table)/sizeof(stk3x3x_als_config_table[0]);
    for(i=0;i<reg_num;i++)
    {
        res = sensor_write_reg(client, stk3x3x_als_config_table[i].address,stk3x3x_als_config_table[i].value);
        if(res < 0)
            {
...
struct sensor_operate light_stk3x3x_ops = {
	...
	.id_data 			= STK3338_PID,//SENSOR_UNKNOW_DATA,		//device id
